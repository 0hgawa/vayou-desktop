...
use crate::mpv::types::{MPV_FORMAT_DOUBLE, MPV_FORMAT_FLAG};
...
pub async fn init_player(
    ...
    events::start_event_loop(mpv_arc, app);
...
pub async fn mpv_command(args: Vec<String>, state: State<'_, MpvState>) -> Result<(), AppError> {
    let strs: Vec<&str> = args.iter().map(std::string::String::as_str).collect();
