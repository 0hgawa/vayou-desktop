...
impl PlaybackService {
    ...
    /// The single definition of "the title of what is playing": the file
    /// name, falling back to mpv's `media-title` for streams that have no
    /// file name. Both the `mpv:file-loaded` event and `get_state` report
    /// this same value, so the frontend never sees the two disagree.
    pub fn current_title(mpv: &MpvPlayer) -> String {
