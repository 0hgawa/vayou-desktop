...
use super::types::{MPV_FORMAT_INT64, MPV_EVENT_FILE_LOADED, MPV_EVENT_NONE, MpvEvent, MPV_FORMAT_DOUBLE, MPV_FORMAT_FLAG};
...
impl MpvPlayer {
    ...
    pub fn new(hwnd: i64) -> Result<Self, MpvError> {
        ...
        Self::set_option_raw(ffi, handle, "wid", MPV_FORMAT_INT64, &raw const hwnd)?;
    ...
    /// Prime hwdec probe + libavcodec + audio output by loading a 1-frame
    /// synthetic clip. Without this the first real file open pays 500ms-2s
    /// of cold-start. Runs before the public event loop is wired, so events
    /// raised here are drained internally and never reach the frontend.
    fn warm_up(&self) {
        if self.command(&["loadfile", "av://lavfi:color=c=black:s=2x2:d=0.04"]).is_err() {
            return;
        }
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(2);
        loop {
            let evt = self.wait_event(0.05);
            if evt.event_id == MPV_EVENT_FILE_LOADED { break; }
            if std::time::Instant::now() >= deadline { break; }
        }
        let _ = self.command(&["stop"]);
        let drain_until = std::time::Instant::now() + std::time::Duration::from_millis(100);
        while std::time::Instant::now() < drain_until {
            if self.wait_event(0.0).event_id == MPV_EVENT_NONE { break; }
        }
        info!("mpv warm-up complete");
    }
    ...
    fn set_option_raw(
        ...
        let c_name = CString::new(name)?;
        let rc =
            unsafe { (ffi.set_option)(handle, c_name.as_ptr(), format, data.cast::<c_void>()) };
        if rc < 0 {
