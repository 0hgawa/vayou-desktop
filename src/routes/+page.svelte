<script lang="ts">
   ...
   *
   * Triggers off `fileEpoch` (incremented on `mpv:file-loaded`) rather than
   * `player.title`, so that re-opening the same file — same title, new
   * playback — still counts as a new file. */
  ...
  $effect(() => {
    ...
    // The title arrives with the load event, which is the only moment it can
    // change. The poll below re-reports the very same value, so the two can
    // never disagree the way the old `mpv:media-title` observer did.
    cleanups.push(listen<string>("mpv:file-loaded", (e) => {
      player.title = e.payload;
