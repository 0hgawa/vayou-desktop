<script lang="ts">
  ...
  // What the row says while the job runs. Extraction has no percentage to
  // report, so it names itself instead of showing a "0%" that never moves.
  const translateStatus = $derived(
    translate.phase === "extracting"
      ? `${t().extractingSubs}…`
      : `${t().translatingSubs} ${translate.total > 0 ? Math.round((translate.progress / translate.total) * 100) : 0}%`,
  );
...
{#snippet transErrorBox()}
  {#if transError}
    <div class="px-3 pb-1.5 flex items-start gap-2">
      <span class="flex-1 text-red-400 text-[12px] leading-snug">{transError}</span>
    ...
    {#if page === "main"}
      ...
      >
        ...
        {#if translate.translating}
          <span class="text-[12px] text-white/55">{translateStatus}</span>
      ...
      <div class="flex-1 overflow-y-auto">
        ...
        {:else if searchResults.length > 0}
          {#each searchResults as sub, i}
            ...
            >
              <span class="text-accent text-[12px] font-medium uppercase tabular-nums w-9 shrink-0 pt-0.5">{sub.lang}</span>
              <span class="flex-1 min-w-0">
                <span class="block text-white/85 text-xs line-clamp-2 leading-snug" title={sub.name}>{wrappable(sub.name)}</span>
                <span class="flex items-center gap-2 mt-0.5 text-[12px] text-white/40">
