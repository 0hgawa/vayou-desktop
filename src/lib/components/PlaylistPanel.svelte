...
{#if visible}
  ...
  <div data-panel class="fixed right-4 bottom-16 z-81 w-80 max-h-[80vh] bg-surface-container/95 backdrop-blur-md border border-white/10 rounded-lg shadow-2xl text-[13px] text-white/90 flex flex-col select-none">
    <!-- Header -->
    <div class="flex items-center px-3 py-2">
      <span class="font-medium text-xs">{t().playlist}</span>
      <span class="text-white/30 text-[12px] ml-2">{items.length} items</span>
    ...
    <div class="flex-1 overflow-y-auto">
      {#each items as item, i}
        <div
          ...
          title={item.filename}
        >
          <span class="w-5 text-[12px] shrink-0 {item.current ? 'text-accent' : 'text-white/25'}">
            {item.current ? "▶" : i + 1}
          </span>
    ...
    <div class="flex items-center px-2 py-1.5 gap-1">
      <button
        ...
        aria-label={t().shuffle} aria-pressed={shuffle} title={t().shuffle} onclick={toggleShuffle}
      >
      ...
      <div class="flex-1"></div>
      <button class="ctrl-btn w-7 h-7 text-white/50" aria-label={t().addFiles} title={t().addFiles} onclick={handleAddFiles}>
        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
      </button>
      <button class="ctrl-btn w-7 h-7 text-white/50" aria-label={t().addFolder} title={t().addFolder} onclick={handleAddFolder}>
      ...
      <button class="ctrl-btn w-7 h-7 text-white/50 hover:text-red-400" aria-label={t().clear} title={t().clear} onclick={handleClear}>
