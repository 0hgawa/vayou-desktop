  ...
  <div class="flex-1 min-w-0">
    {#if below}
      <div class="flex items-baseline justify-between gap-3">
        ...
        {#if value}<div class="text-[12px] text-white/55 tabular-nums shrink-0">{value}</div>{/if}
      </div>
      <div class="mt-1.5">{@render below()}</div>
    {:else}
      <div class="text-[13px] text-white/90 truncate leading-tight">{title}</div>
      {#if value}<div class="text-[12px] text-white/45 truncate mt-0.5 leading-tight">{value}</div>{/if}
    {/if}
  </div>
