import type { Dict } from "./index.svelte";
export default {
  // Context menu
  openFile: "打开文件",
  openUrl: "打开URL",
  pause: "暂停",
  play: "播放",
  stop: "停止",
  subtitles: "字幕",
  audio: "音频",
  speed: "速度",
  chapters: "章节",
  aspectRatio: "画面比例",
  abLoop: "A-B 循环",
  setA: "设置 A 点",
  setB: "设置 B 点",
  screenshot: "截图",
  screenshotSaved: "截图已保存",
  mediaInfo: "媒体信息",
  settings: "设置",
  alwaysOnTop: "置顶",
  sleepTimer: "睡眠定时器",
  cancel: "取消",
  off: "关",
  auto: "自动",

  // Panels
  noSubtitleTracks: "无字幕轨道",
  noAudioTracks: "无音频轨道",
  disableSubs: "禁用",
  loadExternalFile: "打开本地字幕",
  on: "开",
  delay: "延迟",
  translate: "翻译",
  extractingSubs: "正在提取字幕",
  translatingSubs: "正在翻译",
  style: "样式",
  back: "返回",
  reset: "重置",
  font: "字体",
  size: "大小",
  textColor: "文字颜色",
  borderColor: "边框颜色",
  borderSize: "边框大小",
  shadow: "阴影",
  position: "位置",
  text: "文字",
  bold: "??",
  border: "边框",

  // Playlist
  playlist: "播放列表",
  emptyPlaylist: "播放列表为空",
  addFiles: "+ 文件",
  addFolder: "+ 文件夹",
  clear: "清空",
  remove: "移除",
  shuffle: "随机播放",
  repeat: "重复",
  items: "项",

  // Subtitle search
  searchOnline: "在线搜索...",
  searchOnlineTitle: "在线搜索",
  searchSubtitle: "搜索字幕",
  searching: "搜索中...",
  noResults: "无结果",
  downloadFailed: "下载失败",

  // Media Info
  file: "文件",
  resolution: "分辨率",
  videoCodec: "视频编解码器",
  audioCodec: "音频编解码器",
  fps: "FPS",
  videoBitrate: "视频比特率",
  audioBitrate: "音频比特率",
  duration: "时长",
  fileSize: "文件大小",

  // Settings
  general: "常规",
  video: "视频",
  language: "语言",
  preferredAudio: "首选音频",
  preferredSubtitle: "首选字幕",
  defaultVolume: "默认音量",
  defaultSpeed: "默认速度",
  rememberPosition: "记住播放位置",
  autoPlay: "自动播放",
  rememberSelections: "记住音频/字幕",
  subtitleEncoding: "字幕编码",
  color: "颜色",
  brightness: "亮度",
  contrast: "对比度",
  saturation: "饱和度",
  zoom: "缩放",
  normalization: "音频标准化",
  volumeBoost: "音量增强",
  equalizer: "均衡器",
  applyEmbeddedStyles: "应用嵌入样式",
  restoreDefaults: "恢复默认",

  // VideoControls
  previous: "上一个",
  next: "下一个",
  fullscreen: "全屏",
  playbackSpeed: "播放速度",

  // Window
  menu: "菜单",
  minimize: "最小化",
  maximize: "最大化",
  close: "关闭",

  // Shortcuts
  playPause: "播放 / 暂停",
  seekForward5: "快进 (5秒)",
  seekForward30: "快进 (30秒)",
  seekBack5: "快退 (5秒)",
  seekBack30: "快退 (30秒)",
  frameStep: "下一帧",
  frameBack: "上一帧",
  speedUp: "加速",
  speedDown: "减速",
  volumeUp: "增大音量",
  volumeDown: "减小音量",
  mute: "静音",
  unmute: "取消静音",
  cycleSub: "切换字幕",
  cycleAudio: "切换音频",
  shortcuts: "快捷键",
  playback: "播放",
  volume: "音量",
  seekBar: "进度条",
  tracks: "轨道",
  navigation: "导航",
  resetShortcuts: "重置所有快捷键",
  pressKey: "按下一个键...",

  // Page
  dropToPlay: "拖放文件以播放",
  welcomeTitle: "Vayou",
  welcomeSubtitle: "将文件拖放到此处或按",
  toOpen: "打开",

  // About / Updates
  about: "关于",
  videoPlayer: "视频播放器",
  applicationUpdates: "应用更新",
  updateCheckHint: "检查是否有更新的版本。",
  checkForUpdates: "检查更新",
  checkingUpdates: "正在检查...",
  upToDate: "已是最新版本。",
  updateAvailable: "有可用更新:",
  downloadingUpdate: "正在下载并验证...",
  updateInstalled: "更新已安装。重启以应用。",
  updateFailed: "无法更新:",
  openReleasesPage: "打开发布页面",
  install: "安装",
  installing: "正在安装...",
  restartNow: "立即重启",
  restoreDefaultsDesc: "将所有设置恢复为默认值。",
  subErrImageBased: "此字幕为图像格式，无法翻译。",
  subErrRateLimited: "翻译失败 — 服务繁忙，请稍后重试。",
  subErrNoTrack: "未选择字幕轨道。",
  subErrNoEntries: "未找到可翻译的字幕文本。",
  subErrNoFile: "没有正在播放的文件。",
} satisfies Dict;
