import type { Dict } from "./index.svelte";
export default {
  // Context menu
  openFile: "Ouvrir un Fichier",
  openUrl: "Ouvrir URL",
  pause: "Pause",
  play: "Lecture",
  stop: "Arrêter",
  subtitles: "Sous-titres",
  audio: "Audio",
  speed: "Vitesse",
  chapters: "Chapitres",
  aspectRatio: "Rapport d'Aspect",
  abLoop: "Boucle A-B",
  setA: "Définir A",
  setB: "Définir B",
  screenshot: "Capture d'Écran",
  screenshotSaved: "Capture enregistrée",
  mediaInfo: "Info du Média",
  settings: "Paramètres",
  alwaysOnTop: "Toujours au Premier Plan",
  sleepTimer: "Minuteur de Veille",
  cancel: "Annuler",
  off: "Désactivé",
  auto: "Auto",

  // Panels
  noSubtitleTracks: "Aucune piste de sous-titres",
  noAudioTracks: "Aucune piste audio",
  disableSubs: "Désactiver",
  loadExternalFile: "Ouvrir un sous-titre local",
  on: "ACTIVÉ",
  delay: "Délai",
  translate: "Traduire",
  extractingSubs: "Extraction des sous-titres",
  translatingSubs: "Traduction",
  style: "Style",
  back: "Retour",
  reset: "Réinitialiser",
  font: "Police",
  size: "Taille",
  textColor: "Couleur du Texte",
  borderColor: "Couleur de la Bordure",
  borderSize: "Taille de la Bordure",
  shadow: "Ombre",
  position: "Position",
  text: "Texte",
  bold: "Gras",
  border: "Bordure",

  // Playlist
  playlist: "Liste de Lecture",
  emptyPlaylist: "Liste de lecture vide",
  addFiles: "+ Fichiers",
  addFolder: "+ Dossier",
  clear: "Vider",
  remove: "Supprimer",
  shuffle: "Aléatoire",
  repeat: "Répéter",
  items: "éléments",

  // Subtitle search
  searchOnline: "Rechercher en ligne...",
  searchOnlineTitle: "Rechercher en Ligne",
  searchSubtitle: "Rechercher un sous-titre",
  searching: "Recherche en cours...",
  noResults: "Aucun résultat",
  downloadFailed: "Échec du téléchargement",

  // Media Info
  file: "Fichier",
  resolution: "Résolution",
  videoCodec: "Codec Vidéo",
  audioCodec: "Codec Audio",
  fps: "FPS",
  videoBitrate: "Débit Vidéo",
  audioBitrate: "Débit Audio",
  duration: "Durée",
  fileSize: "Taille du Fichier",

  // Settings
  general: "Général",
  video: "Vidéo",
  language: "Langue",
  preferredAudio: "Audio Préféré",
  preferredSubtitle: "Sous-titre Préféré",
  defaultVolume: "Volume par Défaut",
  defaultSpeed: "Vitesse par Défaut",
  rememberPosition: "Mémoriser la Position",
  autoPlay: "Lecture Automatique",
  rememberSelections: "Mémoriser Audio/Sous-titre",
  subtitleEncoding: "Codage du Sous-titre",
  color: "Couleur",
  brightness: "Luminosité",
  contrast: "Contraste",
  saturation: "Saturation",
  zoom: "Zoom",
  normalization: "Normalisation",
  volumeBoost: "Amplification du Volume",
  equalizer: "Égaliseur",
  applyEmbeddedStyles: "Appliquer les Styles Intégrés",
  restoreDefaults: "Restaurer les Valeurs par Défaut",

  // VideoControls
  previous: "Précédent",
  next: "Suivant",
  fullscreen: "Plein Écran",
  playbackSpeed: "Vitesse de lecture",

  // Window
  menu: "Menu",
  minimize: "Réduire",
  maximize: "Agrandir",
  close: "Fermer",

  // Shortcuts
  playPause: "Lecture / Pause",
  seekForward5: "Avancer (5s)",
  seekForward30: "Avancer (30s)",
  seekBack5: "Reculer (5s)",
  seekBack30: "Reculer (30s)",
  frameStep: "Image Suivante",
  frameBack: "Image Précédente",
  speedUp: "Accélérer",
  speedDown: "Ralentir",
  volumeUp: "Augmenter le Volume",
  volumeDown: "Baisser le Volume",
  mute: "Muet",
  unmute: "Réactiver le son",
  cycleSub: "Changer les Sous-titres",
  cycleAudio: "Changer la Piste Audio",
  shortcuts: "Raccourcis",
  playback: "Lecture",
  volume: "Volume",
  seekBar: "Barre de progression",
  tracks: "Pistes",
  navigation: "Navigation",
  resetShortcuts: "Réinitialiser Tous les Raccourcis",
  pressKey: "Appuyez sur une touche...",

  // Page
  dropToPlay: "Déposez le fichier pour le lire",
  welcomeTitle: "Vayou",
  welcomeSubtitle: "Déposez un fichier ici ou appuyez sur",
  toOpen: "pour ouvrir",

  // About / Updates
  about: "À propos",
  videoPlayer: "Lecteur vidéo",
  applicationUpdates: "Mises à jour de l'application",
  updateCheckHint: "Vérifiez si une version plus récente est disponible.",
  checkForUpdates: "Rechercher des mises à jour",
  checkingUpdates: "Vérification...",
  upToDate: "Vous avez la dernière version.",
  updateAvailable: "Mise à jour disponible :",
  downloadingUpdate: "Téléchargement et vérification...",
  updateInstalled: "Mise à jour installée. Redémarrez pour appliquer.",
  updateFailed: "Échec de la mise à jour :",
  openReleasesPage: "Ouvrir la page des versions",
  install: "Installer",
  installing: "Installation...",
  restartNow: "Redémarrer maintenant",
  restoreDefaultsDesc: "Réinitialiser tous les paramètres à leurs valeurs par défaut.",
  subErrImageBased: "Ces sous-titres sont basés sur une image et ne peuvent pas être traduits.",
  subErrRateLimited: "Échec de la traduction — le service est occupé. Réessayez dans un instant.",
  subErrNoTrack: "Aucune piste de sous-titres sélectionnée.",
  subErrNoEntries: "Aucun texte de sous-titres à traduire.",
  subErrNoFile: "Aucun fichier en lecture.",
} satisfies Dict;
