import type { Dict } from "./index.svelte";
export default {
  // Context menu
  openFile: "파일 열기",
  openUrl: "URL 열기",
  pause: "일시정지",
  play: "재생",
  stop: "정지",
  subtitles: "자막",
  audio: "오디오",
  speed: "속도",
  chapters: "챕터",
  aspectRatio: "화면 비율",
  abLoop: "A-B 반복",
  setA: "A 지점 설정",
  setB: "B 지점 설정",
  screenshot: "스크린샷",
  screenshotSaved: "스크린샷이 저장되었습니다",
  mediaInfo: "미디어 정보",
  settings: "설정",
  alwaysOnTop: "항상 위에",
  sleepTimer: "취침 타이머",
  cancel: "취소",
  off: "끔",
  auto: "자동",

  // Panels
  noSubtitleTracks: "자막 트랙 없음",
  noAudioTracks: "오디오 트랙 없음",
  disableSubs: "비활성화",
  loadExternalFile: "로컬 자막 열기",
  on: "켬",
  delay: "지연",
  translate: "번역",
  extractingSubs: "자막 추출 중",
  translatingSubs: "번역 중",
  style: "스타일",
  back: "뒤로",
  reset: "초기화",
  font: "글꼴",
  size: "크기",
  textColor: "글자 색상",
  borderColor: "테두리 색상",
  borderSize: "테두리 크기",
  shadow: "그림자",
  position: "위치",
  text: "글자",
  bold: "??",
  border: "테두리",

  // Playlist
  playlist: "재생목록",
  emptyPlaylist: "재생목록이 비어 있습니다",
  addFiles: "+ 파일",
  addFolder: "+ 폴더",
  clear: "비우기",
  remove: "제거",
  shuffle: "셔플",
  repeat: "반복",
  items: "항목",

  // Subtitle search
  searchOnline: "온라인 검색...",
  searchOnlineTitle: "온라인 검색",
  searchSubtitle: "자막 검색",
  searching: "검색 중...",
  noResults: "결과 없음",
  downloadFailed: "다운로드 실패",

  // Media Info
  file: "파일",
  resolution: "해상도",
  videoCodec: "비디오 코덱",
  audioCodec: "오디오 코덱",
  fps: "FPS",
  videoBitrate: "비디오 비트레이트",
  audioBitrate: "오디오 비트레이트",
  duration: "재생 시간",
  fileSize: "파일 크기",

  // Settings
  general: "일반",
  video: "영상",
  language: "언어",
  preferredAudio: "선호 오디오",
  preferredSubtitle: "선호 자막",
  defaultVolume: "기본 음량",
  defaultSpeed: "기본 속도",
  rememberPosition: "재생 위치 기억",
  autoPlay: "자동 재생",
  rememberSelections: "오디오/자막 기억",
  subtitleEncoding: "자막 인코딩",
  color: "색상",
  brightness: "밝기",
  contrast: "대비",
  saturation: "채도",
  zoom: "확대/축소",
  normalization: "정규화",
  volumeBoost: "볼륨 부스트",
  equalizer: "이퀄라이저",
  applyEmbeddedStyles: "내장 스타일 적용",
  restoreDefaults: "기본값 복원",

  // VideoControls
  previous: "이전",
  next: "다음",
  fullscreen: "전체 화면",
  playbackSpeed: "재생 속도",

  // Window
  menu: "메뉴",
  minimize: "최소화",
  maximize: "최대화",
  close: "닫기",

  // Shortcuts
  playPause: "재생 / 일시정지",
  seekForward5: "5초 앞으로",
  seekForward30: "30초 앞으로",
  seekBack5: "5초 뒤로",
  seekBack30: "30초 뒤로",
  frameStep: "다음 프레임",
  frameBack: "이전 프레임",
  speedUp: "속도 높이기",
  speedDown: "속도 낮추기",
  volumeUp: "음량 높이기",
  volumeDown: "음량 낮추기",
  mute: "음소거",
  unmute: "음소거 해제",
  cycleSub: "자막 전환",
  cycleAudio: "오디오 전환",
  shortcuts: "단축키",
  playback: "재생",
  volume: "음량",
  seekBar: "탐색 막대",
  tracks: "트랙",
  navigation: "탐색",
  resetShortcuts: "모든 단축키 초기화",
  pressKey: "키를 누르세요...",

  // Page
  dropToPlay: "파일을 놓아서 재생",
  welcomeTitle: "Vayou",
  welcomeSubtitle: "여기에 파일을 놓거나",
  toOpen: "눌러서 열기",

  // About / Updates
  about: "정보",
  videoPlayer: "비디오 플레이어",
  applicationUpdates: "앱 업데이트",
  updateCheckHint: "새 버전이 있는지 확인합니다.",
  checkForUpdates: "업데이트 확인",
  checkingUpdates: "확인 중...",
  upToDate: "최신 버전입니다.",
  updateAvailable: "업데이트 사용 가능:",
  downloadingUpdate: "다운로드 및 확인 중...",
  updateInstalled: "업데이트가 설치되었습니다. 적용하려면 다시 시작하세요.",
  updateFailed: "업데이트할 수 없습니다:",
  openReleasesPage: "릴리스 페이지 열기",
  install: "설치",
  installing: "설치 중...",
  restartNow: "지금 다시 시작",
  restoreDefaultsDesc: "모든 설정을 기본값으로 되돌립니다.",
  subErrImageBased: "이 자막은 이미지 기반이라 번역할 수 없습니다.",
  subErrRateLimited: "번역 실패 — 서비스가 혼잡합니다. 잠시 후 다시 시도하세요.",
  subErrNoTrack: "선택된 자막 트랙이 없습니다.",
  subErrNoEntries: "번역할 자막 텍스트를 찾을 수 없습니다.",
  subErrNoFile: "재생 중인 파일이 없습니다.",
} satisfies Dict;
