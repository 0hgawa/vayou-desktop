...
/**
 * Whether Shift is a meaningful part of a combo for this key.
 *
 * A symbol key already bakes Shift in on most layouts — "+" IS Shift+"=" —
 * so the browser reports `key: "+"` with `shiftKey: true`. Folding Shift into
 * the combo there would build "shift++", which no binding can ever match, and
 * the "+" default (speed up) would silently never fire. Named keys
 * (ArrowLeft, F5) and letters keep Shift, where it genuinely distinguishes
 * one binding from another.
 *
 * Mirrors the Slint build's `shift_matters`.
 */
